...
const TableData = ({ data, showTableHeader = false, showTableSearch = false, showTableFilter = false, onDelete = () => { } }) => {
  ...
  return (
    <div className="p-8 bg-white">
      ...
      <div className="overflow-x-auto">
                ...
                <tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-slate-100'}>
                  ...
                  <td className="py-2 px-6 space-x-2"> {/* KODE BARU */}
                    <Link to="/edit-data/:id">
                      <button className="px-3 py-1 bg-blue-500 text-white rounded cursor-pointer"> Edit</button>
                    </Link>
                    <button className="px-3 py-1 bg-yellow-500 text-white rounded cursor-pointer"> Detail</button>
                    <button onClick={() => {
                      console.log(`Click delete id: ${row.id}`);
                      openDeleteModal(row)
                    }} className="px-3 py-1 bg-red-500 text-white rounded cursor-pointer"> Hapus</button>
                  </td>
                </tr>
              ))
            ) : (
        ...
        <div className={`w-20 h-20 max-md:w-14 max-md:h-14 rounded-full flex items-center justify-center`}>
          <div className={`w-16 h-16 max-md:w-11 max-md:h-11  rounded-full flex items-center justify-center`}>
            A
          </div>
        </div>
